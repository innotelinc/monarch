...
| Guide (EPG) | `monarch-init` + `iptv` container | channel list at `/opt/epg/channels.xml` (assembled from `LIVETV_EPG_SITES`); guide grabbed twice a day and served at `http://iptv:3000/guide.xml`. The grabber holds a site's listings in memory and Node's default heap is too small for a large list, so `NODE_OPTIONS=--max-old-space-size=6144` is set in `docker-compose.yml` |
