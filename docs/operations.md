...
| Guide (EPG) | `monarch-init` + `iptv` container | channel list at `/opt/epg/channels.xml` (assembled from `LIVETV_EPG_SITES`); guide grabbed twice a day and served at `http://iptv:3000/guide.xml`. The grabber holds every listing it has fetched, so its peak is days x channels and the run has not fitted twice: three days aborted at Node's default ceiling (~2.4 GB) and two days at a 5 GB heap still held 5.07 GB of live programme data. `DAYS=1` (re-grabbed twice a day, so Jellyfin always has 12+ hours ahead), `NODE_OPTIONS=--max-old-space-size=4096` and `mem_limit: 6g` in `docker-compose.yml` keep the run inside this container, so a runaway grab dies in its own cgroup instead of taking the host's OOM killer down on Jellyfin and the *arr stack. `DAYS` is the only way to pass `--days`: the image's pm2 config never does |
