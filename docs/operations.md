...
| Guide (EPG) | `monarch-init` + `iptv` container | channel list at `/opt/epg/channels.xml` (assembled from `LIVETV_EPG_SITES`); guide grabbed twice a day and served at `http://iptv:3000/guide.xml`. The grabber holds the whole run in memory, so peak use scales with days x channels; `DAYS=2`, `NODE_OPTIONS=--max-old-space-size=5120` and `mem_limit: 6g` in `docker-compose.yml` are sized from the killed runs (Node's default ceiling died at ~2.4 GB, a 4 GB limit died at 83%), so the grab now stops inside this container instead of taking the host's OOM killer down on Jellyfin and the *arr stack |
