...
| Guide (EPG) | `monarch-init`, `monarch-livetv-guide.timer`, `iptv` container | channel list at `/opt/epg/channels.xml` (assembled from `LIVETV_EPG_SITES`); guide served at `http://iptv:3000/guide.xml`. The grabber holds every listing it has fetched until it writes the file at the end, so one run's peak is days x channels and no single run of the whole channel list has ever finished here (three days aborted at Node's default heap, two days at 5 GB still held 5.07 GB *live*). `scripts/shard-livetv-guide.py` (run by `monarch-livetv-guide.timer` at 00:00/12:00) therefore grabs one **site** at a time — split further at `--max-channels 900` — and merges the parts into the same `guide.xml`, which is what lets the horizon be three days instead of one. The container is a server only (`init/iptv-pm2.config.js` is mounted over the image's own pm2 config, so its cron grab cannot race the shard), with `NODE_OPTIONS=--max-old-space-size=3072` and `mem_limit: 4g` sized for one part. A part that runs out of heap is halved and retried (`MIN_CHANNELS 100`, so up to `epg.iptvx.one-1-split1-split2-split3.xml`); a site whose parts keep failing is asked once whether **one channel** fits, and dropped for that run if it does not. `epg.iptvx.one` (729 channels, 679 of them served by no other site) fails that probe as of 2026-09-20 — a single channel exhausted the 3 GB heap in 214 s at `MAX_CONNECTIONS=1` — so the guide covers the other nine sites and the run reports it as missing; it is re-probed every run and needs no change to come back. That drop is reported **without** failing the run: it is the policy working, and counting it as a failure made `monarch-livetv-guide.service` go red on every timer tick until the red meant nothing. A part that fails for any other reason still exits 1 |
